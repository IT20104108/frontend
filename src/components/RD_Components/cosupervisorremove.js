...
retrievecosupervisorDetails(){
    axios.get("https://afbackend.herokuapp.com/usersremove/getcosupervisor").then(res=>{
        if(res.data.status){
            this.setState({
...
onDelete = (cosupervisorID) => {
    ...
    if (window.confirm('Are you sure you wish to delete this details?')) {
      axios.delete(`https://afbackend.herokuapp.com/usersremove/cosupervisordelete/${cosupervisorID}`).then((res) => {

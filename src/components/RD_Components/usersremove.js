...
retrievepanelmemberDetails(){
    axios.get("https://afbackend.herokuapp.com/usersremove/getpanelmember").then(res=>{
        if(res.data.status){
            this.setState({
...
handleSearchArea = (e) => {
  ...
  axios.get("https://afbackend.herokuapp.com/usersremove/getstudent").then(res => {
    if (res.data.success) {
...
onDelete = (panelmemberID) => {
  ...
  if (window.confirm('Are you sure you wish to delete this details?')) {
    axios.delete(`https://afbackend.herokuapp.com/usersremove/panelmemberdelete/${panelmemberID}`).then((res) => {

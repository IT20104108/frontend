...
retrievesupervisorDetails(){
    axios.get("https://afbackend.herokuapp.com/usersremove/getsupervisor").then(res=>{
        if(res.data.status){
            this.setState({
...
onDelete = (supervisorID) => {
    ...
    if (window.confirm('Are you sure you wish to delete this details?')) {
      axios.delete(`https://afbackend.herokuapp.com/usersremove/supervisordelete/${supervisorID}`).then((res) => {
